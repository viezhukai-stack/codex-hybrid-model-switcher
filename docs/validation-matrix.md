...
| Windows netdisk one-click installer | Passed prior real UI canaries; v2.18.3 package and physical update gates passed | Optional | Real UI | The v2.18.3 netdisk setup includes portable Python and the current project payload, guarded `APPLY`, daily Hot Router, official restore, Browser/CLI repair, and device-code account entries. It now blocks an older Codex launch when `pending_registration=true`, self-heals stale `python312._pth` release paths, and keeps the bounded post-start Browser phase. SHA256 `625cfd421ec19d497aab283f4db54501f3a0dd6732b13077bd77532899dd689c`. Windows canary A reported a healthy current registration; Windows canary B correctly detected `26.721.4979.0` staged above `26.715.10079.0` and set `launch_safe=false`. |
| Windows full local netdisk package | Passed v2.15.5 HL Hyper-V VM canary; v2.18.3 package audit and guarded physical canaries passed | Passed | Passed | `Codex-Hybrid-Windows-Full-Local-Setup-v2.18.3.zip` is the current private package for llama.cpp, VC++ Runtime, portable Python, and `payload/models/local-gemma`; SHA256 `14c75b47c4bbe8018258fc4e7323e0263c6ea2c9454d50b6b81ec42561ac1c96`. ZIP integrity, runtime/model manifests, complete Responses conversion, staged-AppX gate, portable-Python helper, source payload, and redaction checks passed. Historical full-local UI canaries still cover text, vision, account, plugins, projects, mutually exclusive local runtime, and idle exit. |
...
| macOS full local netdisk package | Passed v2.17.3/v2.18.0 UI canaries and v2.18.3 isolated Router canary | Passed | Passed | `Codex-Hybrid-macOS-Full-Local-Setup-v2.18.3.zip` is the current private full-local package; SHA256 `305606d0c282112f4ecbd7d71676e309721532955a00e7c4763821d5b7897c0e`. It passed ZIP integrity, model/mmproj/runtime manifests, payload redaction, and an isolated `19132` live check with 59 models. Gemini High preserved reasoning, function-call arguments, usage, terminal status, and `[DONE]` after one bounded empty-output retry. The prior controlled UI canaries remain the account/plugin/project preservation baseline. |
| Shared v2.18.3 hot router | Passed | Passed | Passed on isolated macOS plus Windows source canaries | Unit and live checks cover dynamic catalogs, local merge, model routing, complete Responses JSON-to-SSE conversion, Chat Completions tool-call conversion, bounded `Retry-After` 429 handling, and bounded empty-success retries for Gemini High. The 2026-07-26 isolated Mac canary returned 59 models and preserved a real `reasoning + function_call` response. Windows candidate probes confirmed healthy current registration on canary A and correct staged-update blocking on canary B. The router source is shared by macOS and Windows packages. |
