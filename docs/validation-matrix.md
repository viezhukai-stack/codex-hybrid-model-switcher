...
| Windows full local netdisk package | Passed v2.15.5 HL Hyper-V VM canary; current v2.17.5 source audit passed | Passed | Passed | `Codex-Hybrid-Windows-Full-Local-Setup-v2.17.5.zip` is the current private netdisk package path for bundling `payload/llama.cpp`, `payload/vcredist/vc_redist.x64.exe`, and `payload/models/local-gemma`. The v2.15.5 HL VM canary used no cloud API key, copied the bundled local model into local app data, passed local text smoke with `OK`, passed vision smoke with `Red`, completed guarded dry-run and real `APPLY`, preserved `auth.json`, `models_cache.json`, and `state_5.sqlite`, kept account information, plugins, project conversations, and a new responding test chat visible in Codex Desktop. v2.17.5 adds the Windows 2.0 hot-router launcher path while keeping the same generic `local/gemma` package default. |
...
| macOS full local netdisk package | Passed v2.17.3 Mac UI canary; current v2.17.4 package audit passed | Passed | Passed | `Codex-Hybrid-macOS-Full-Local-Setup-v2.17.4.zip` is the current private netdisk package path for bundling both macOS x64 and arm64 llama.cpp runtimes plus `payload/models/local-gemma`. The 2026-07-04 Mac canary confirmed account visibility, plugin visibility, project conversations, and a responding test chat after full-local package use and main-profile restore. v2.17.4 keeps the same full-local runtime path and cleans the netdisk payload. See `docs/macos-full-local-pack-canary.md`. |
