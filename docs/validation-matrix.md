...
| Windows netdisk one-click installer | Passed v2.14.8 canary; v2.15.x local package path added | Optional | Real UI | `installer/windows` and `docs/windows-one-click-installer.md` define the beginner netdisk setup zip. It opens the official Codex app page when Codex is missing, includes portable Python by default, uses bundled project payload without requiring GitHub project download, can prefill provider settings from `provider-preset.json`, can use bundled or official-download llama.cpp assets, can auto-detect `payload/models/local-gemma` for local-only setup, reports install-drive free space, can optionally unify openai history rows and matching `sessions/*.jsonl` metadata into the custom bucket and active model after backups, installs a restore-to-official launcher, writes diagnostics on request, and defaults to guarded dry-run before explicit `APPLY` confirmation. HL Hyper-V VM canary confirmed account visibility, plugin visibility, temporary chats, project chats, and a responding test chat after unification. |
| Windows full local netdisk package | Passed v2.15.5 HL Hyper-V VM canary; v2.18.0 package and HL read-only audits passed | Passed | Passed on prior canary | `Codex-Hybrid-Windows-Full-Local-Setup-v2.18.0.zip` is the current private netdisk package path for bundling `payload/llama.cpp`, `payload/vcredist/vc_redist.x64.exe`, portable Python, and `payload/models/local-gemma`; SHA256 `cb057af4cd7adae07ddaa54b2dd35b333d63620aa1cb97de2765bfeca3f5e9a8`. The v2.15.5 HL VM canary used no cloud API key, passed local text smoke with `OK`, passed vision smoke with `Red`, completed guarded `APPLY`, preserved protected files, and kept account, plugins, project conversations, and a responding test chat visible. The v2.18.0 package audit verifies the shared 2.0 payload, ZIP integrity, runtimes, model manifest, and redaction. A read-only check on the upgraded HL app confirmed `ChatGPT.exe` detection, PowerShell 5 parsing, dynamic OpenAI.Codex AppID resolution, 48 catalog models including `gpt-5.6-sol`, unchanged protected files, and graceful `WARN` output when the WindowsApps CLI cannot execute through SSH. A fresh v2.18.0 Windows UI canary remains a release follow-up rather than a claim of these read-only audits. |
...
| macOS full local netdisk package | Passed v2.17.3 and v2.18.0 Mac UI canaries | Passed | Passed | `Codex-Hybrid-macOS-Full-Local-Setup-v2.18.0.zip` is the current private netdisk package path for bundling both macOS x64 and arm64 llama.cpp runtimes plus `payload/models/local-gemma`; SHA256 `1636f56cf27b1008967d34f4f10f84ee76fcd61a69c46c990676cca7cfa49334`. The 2026-07-04 Mac canary confirmed account, plugins, project conversations, a responding test chat, and successful main-profile restore. The v2.18.0 isolated flow passed local text `OK`, vision `Red`, guarded dry-run, and local-only catalog checks; the rebuilt package audit verifies the shared hot-router source, bundle-id-first ChatGPT/Codex discovery, 2.0 launch/enable/restore entries, ZIP integrity, model manifest, and package redaction. The final installed v2.18 router was checked on isolated port `19132` with cloud `gpt-5.6-sol` returning `OK`, the configured local model returning `OK`, and local vision returning `Red`. The controlled v2.18.0 UI canary confirmed account, plugins/MCP, project conversations, `gpt-5.6-sol`, the configured local model, and normal replies after upgraded app restart. `auth.json` and `state_5.sqlite` stayed unchanged; `models_cache.json` refreshed during app launch and is recorded as an upgraded-app cache observation. See `docs/macos-full-local-pack-canary.md`. |
| Shared v2.18.0 hot router | Passed | Passed | Passed on macOS; Windows UI canary follow-up | Unit and live isolated checks verify dynamic cloud catalogs, future model IDs, `default_cloud_provider_id`, same-endpoint fallback, multi-endpoint ambiguity refusal, private hidden/alias rules, local model merge, no-store catalog responses, and removal of conditional ETag headers. A read-only check of the active Mac `19032` catalog returned 46 models including `gpt-5.6-sol`. The final installed shared router on isolated port `19132` returned 46 models including `gpt-5.6-sol` and the configured local model, routed `gpt-5.6-sol` to `OK`, routed the local text request to `OK`, and routed the local image request to `Red`. The controlled macOS v2.18.0 UI canary confirmed the same model list and normal replies after restart. The router source is shared by macOS and Windows packages. |
