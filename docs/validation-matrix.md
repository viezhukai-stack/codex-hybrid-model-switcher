...
| Windows netdisk one-click installer | Pending v2.13.2 canary | Optional | Package | `installer/windows` and `docs/windows-one-click-installer.md` define the beginner netdisk setup zip. It opens the official Codex app page when Codex is missing, installs Python with `winget` or uses bundled portable Python, uses bundled project payload without requiring GitHub project download, can prefill provider settings from `provider-preset.json`, can use bundled or official-download llama.cpp assets, writes diagnostics on request, and defaults to guarded dry-run before explicit `APPLY` confirmation. |
