...
| Windows full local netdisk package | Passed v2.15.5 HL Hyper-V VM canary; v2.18.0 package audit passed | Passed | Passed on prior canary | `Codex-Hybrid-Windows-Full-Local-Setup-v2.18.0.zip` is the current private netdisk package path for bundling `payload/llama.cpp`, `payload/vcredist/vc_redist.x64.exe`, portable Python, and `payload/models/local-gemma`. The v2.15.5 HL VM canary used no cloud API key, passed local text smoke with `OK`, passed vision smoke with `Red`, completed guarded `APPLY`, preserved protected files, and kept account, plugins, project conversations, and a responding test chat visible. The v2.18.0 package audit additionally verifies `ChatGPT.exe` running protection, dynamic OpenAI.Codex AppID discovery, the shared 2.0 hot-router payload, ZIP integrity, required runtimes, model manifest, and package redaction. A fresh v2.18.0 Windows UI canary remains a release follow-up rather than a claim of this package audit. |
