...
| Windows v2.18.8 netdisk one-click package | Passed ZIP, structure, version, CRC, and sensitive-content audit | Optional | Package verified | `Codex-Hybrid-Windows-Netdisk-Setup-v2.18.8.zip` contains portable Python, the current Hot Router and maintenance entries, the complete CLI-bundle update repair, and the CLI `0.149.0+` Desktop-owned `openai-bundled` compatibility path. SHA256 `148e68d2e4fcb42f11555895c62bf03b9f7d66b67b3da8acf0848e395a0900b4`; 146 entries. This release audit is package/simulation evidence and does not replace the earlier physical Windows canaries. |
| Windows v2.18.8 full-local package | Passed ZIP, structure, version, CRC, model-manifest, and sensitive-content audit | Reused verified Gemma/mmproj and llama.cpp payload | Package verified | `Codex-Hybrid-Windows-Full-Local-Setup-v2.18.8.zip` contains portable Python, VC++ Runtime, Windows llama.cpp, and the unchanged verified `local/gemma` model payload. SHA256 `6ab3d5f399db13ff3e4c5627408c46224077a0b99414dac7fde422e9d92e29a3`; 203 entries. Embedded model SHA256 values match the v2.18.7 verified payload. |
| macOS v2.18.8 full-local package | Passed ZIP, structure, version, CRC, executable-mode, model-manifest, and sensitive-content audit | Reused verified Gemma/mmproj and dual-architecture llama.cpp payload | Package verified | `Codex-Hybrid-macOS-Full-Local-Setup-v2.18.8.zip` contains x64 and arm64 llama.cpp runtimes plus the unchanged verified `local/gemma` payload. SHA256 `5ec244db0123159860e9ad0f38923f92e7b77d6f739d35907a6aebc3cc9eede2`; 235 entries. The Windows-only update transaction remains inert on macOS. |
| Shared v2.18.8 source and release gate | 253 tests, compileall, security scan, release acceptance, stock handoff, agent drill, and real-canary template passed | Existing local runtimes preserved | Simulated | Coverage includes CLI `0.149.0+` reserved marketplace handling, complete five-file CLI drift detection, staging-lock retries, failed-swap rollback, verified config restore fallback, strict catalogs, Responses conversion, WebSocket Live routing, package exclusions, and protected-state invariants. |
