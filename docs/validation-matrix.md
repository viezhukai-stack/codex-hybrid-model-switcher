...
| Windows netdisk one-click installer | Passed prior real UI canaries; v2.18.1 update repair passed on a physical Windows Hybrid 2.0 setup | Optional | Real UI | `installer/windows` and `docs/windows-one-click-installer.md` define the beginner netdisk setup zip. It includes portable Python and the project payload, supports cloud or bundled local setup, keeps guarded `APPLY`, and installs daily Hot Router, official restore, Browser/CLI repair, and device-code account entries. v2.18.1 adds `windows-update-ensure`: the daily launcher automatically refreshes the complete hash-verified AppX CLI bundle only for a supported version change while Codex is closed, then reruns the launch gate. Other failures stop without a service, scheduled task, watchdog, or restart loop. Netdisk setup SHA256 `a57ff406f6d5ae58896eb09ec6bf7a9776c09084f17cf4bf9977843ea817fc5e`. |
| Windows full local netdisk package | Passed v2.15.5 HL Hyper-V VM canary; v2.18.1 package audit and physical update canary passed | Passed | Passed | `Codex-Hybrid-Windows-Full-Local-Setup-v2.18.1.zip` is the current private netdisk package for `payload/llama.cpp`, `payload/vcredist/vc_redist.x64.exe`, portable Python, and `payload/models/local-gemma`; SHA256 `47fd0c231372338d1cedde35d08a1d2d2b48f1d1df4c046c5b6d4d7f37a58232`. The historical full-local VM canary passed text, vision, guarded apply, protected-file checks, account, plugins, and projects. The rebuilt v2.18.1 ZIP passed integrity, runtime, model-manifest, source-payload, automatic-update-guard, and redaction audits. The physical 2026-07-22 update canary covered two consecutive Codex AppX versions, refreshed the complete five-file CLI bundle, restored Browser/Chrome DOM access, preserved 100 `custom` tasks with database `quick_check=ok`, kept `19030/19032` healthy, and left `19031` stopped while idle. See `docs/windows-codex-update-canary.md` and `docs/windows-full-local-pack-canary.md`. |
