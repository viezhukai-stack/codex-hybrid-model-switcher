...
| Windows full local netdisk package | Passed v2.15.5 HL Hyper-V VM canary; current v2.17.4 package audit passed | Passed | Passed | `Codex-Hybrid-Windows-Full-Local-Setup-v2.17.4.zip` is the current private netdisk package path for bundling `payload/llama.cpp`, `payload/vcredist/vc_redist.x64.exe`, and `payload/models/local-gemma`. The v2.15.5 HL VM canary used no cloud API key, copied the bundled local model into local app data, passed local text smoke with `OK`, passed vision smoke with `Red`, completed guarded dry-run and real `APPLY`, preserved `auth.json`, `models_cache.json`, and `state_5.sqlite`, kept account information, plugins, project conversations, and a new responding test chat visible in Codex Desktop. v2.17.4 keeps the same full-local runtime path and cleans the netdisk payload. |
