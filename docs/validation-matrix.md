...
| Windows netdisk one-click installer | Passed prior real UI canaries; v2.18.2 update repair passed on a physical Windows Hybrid 2.0 setup | Optional | Real UI | `installer/windows` and `docs/windows-one-click-installer.md` define the beginner netdisk setup zip. It includes portable Python and the project payload, supports cloud or bundled local setup, keeps guarded `APPLY`, and installs daily Hot Router, official restore, Browser/CLI repair, and device-code account entries. The daily launcher runs the closed-app `windows-update-ensure` phase, then a bounded `windows-browser-ensure` phase after Codex feature initialization. Other failures stop without a service, scheduled task, watchdog, or restart loop. Netdisk setup v2.18.2 SHA256 `6865dcda03715cb76a990cb5df7f38e11b39b739dfea3ae8fc23a4780b2c9fef`. |
| Windows full local netdisk package | Passed v2.15.5 HL Hyper-V VM canary; v2.18.2 package audit and physical update canary passed | Passed | Passed | `Codex-Hybrid-Windows-Full-Local-Setup-v2.18.2.zip` is the current private netdisk package for `payload/llama.cpp`, `payload/vcredist/vc_redist.x64.exe`, portable Python, and `payload/models/local-gemma`; SHA256 `96355131fccf772942571ade066065466c5e30aa70ca4e97d40cfba68d12b7da`. The historical full-local VM canary passed text, vision, guarded apply, protected-file checks, account, plugins, and projects. The rebuilt v2.18.2 ZIP passed integrity, runtime, model-manifest, source-payload, two-phase update guard, and redaction audits. The physical update canary covered two consecutive Codex AppX versions, refreshed the complete five-file CLI bundle, restored Browser/Chrome after feature initialization, preserved 100 `custom` tasks with database `quick_check=ok`, kept `19030/19032` healthy, and left `19031` stopped while idle. See `docs/windows-codex-update-canary.md` and `docs/windows-full-local-pack-canary.md`. |
