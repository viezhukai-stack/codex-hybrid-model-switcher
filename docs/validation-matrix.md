...
| Windows netdisk one-click installer | Passed v2.18.4 package and physical HL/Kevin canaries | Optional | Real UI | `Codex-Hybrid-Windows-Netdisk-Setup-v2.18.4.zip` contains portable Python, the shared WebSocket Hot Router, strict catalog support, Browser/CLI and account maintenance, plus explicit Live audio doctor/repair/restore; SHA256 `d98a1ade2e4c7dac5dc3dceb5515963d8682206cc53436b157d0d54bfcd40baf`. Package-source canaries returned 12 models on HL and 11 on Kevin, text 200, an upstream WebSocket response, healthy audio status, unchanged protected files, and successful daily-launcher reopen. |
| Windows full local netdisk package | Passed historical HL local-runtime canary plus v2.18.4 package audit and physical source canaries | Passed | Passed | `Codex-Hybrid-Windows-Full-Local-Setup-v2.18.4.zip` is the current private package for llama.cpp, VC++ Runtime, portable Python, and `payload/models/local-gemma`; SHA256 `d4202240e2328c256b3f22ded800cc08fa2930b7ccdf809c570d872ab2b9de04`. ZIP integrity, runtime/model manifests, current WebSocket source, Live audio entry, executable/runtime modes, and redaction checks passed. The local payload is reused from the verified v2.18.3 package; historical UI canaries continue to cover text, vision, mutually exclusive local runtime, and idle exit. |
...
| macOS full local netdisk package | Passed prior UI canaries plus v2.18.4 isolated package canary | Passed | Passed | `Codex-Hybrid-macOS-Full-Local-Setup-v2.18.4.zip` is the current private full-local package; SHA256 `5d7e9ee98116d0b397eeb7c059481c8380abcb2eac62997340565c9a9a6a7dff`. It passed ZIP integrity, model/mmproj/runtime manifests and executable modes, payload redaction, plus an isolated `19132` canary with the curated 10-model catalog, text 200, and WebSocket forwarding without changing the active Mac task. |
| Shared v2.18.4 hot router | Passed | Passed | Passed on isolated macOS and physical Windows package canaries | The 220-test suite covers strict cloud/local catalogs, local metadata, Responses conversion, retries, and HTTP/1.1 WebSocket tunneling. Final package canaries returned 10 models on Mac, 12 on HL, and 11 on Kevin; all returned text 200 and forwarded the WebSocket upgrade. Prior real Live UI sessions on all three machines reached HTTP 101, while Router logs retained only status and byte metadata. |
