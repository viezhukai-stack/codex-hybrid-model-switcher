...
| Windows full local netdisk package | Passed v2.15.5 HL Hyper-V VM canary | Passed | Passed | `Codex-Hybrid-Windows-Full-Local-Setup-v2.15.5.zip` is the private netdisk package path for bundling `payload/llama.cpp`, `payload/vcredist/vc_redist.x64.exe`, and `payload/models/local-gemma`. The v2.15.5 HL VM canary used no cloud API key, copied the bundled local model into local app data, passed local text smoke with `OK`, passed vision smoke with `Red`, completed guarded dry-run and real `APPLY`, preserved `auth.json`, `models_cache.json`, and `state_5.sqlite`, kept account information, plugins, project conversations, and a new responding test chat visible in Codex Desktop. |
...
| macOS full local netdisk package | Passed v2.17.3 Mac UI canary | Passed | Passed | `Codex-Hybrid-macOS-Full-Local-Setup-v2.17.3.zip` is the private netdisk package path for bundling both macOS x64 and arm64 llama.cpp runtimes plus `payload/models/local-gemma`. The 2026-07-04 Mac canary confirmed account visibility, plugin visibility, project conversations, and a responding test chat after full-local package use and main-profile restore. See `docs/macos-full-local-pack-canary.md`. |
