...
1. 先安装并登录原版 ChatGPT/Codex Desktop。
   官方页面：https://developers.openai.com/codex/app
   新版 macOS 应用可能在“应用程序”里显示为 ChatGPT.app，旧版可能显示为 Codex.app。
...
5. 安装器会创建私有配置、检查配置、执行安全 dry-run，并在桌面安装
   Start Codex Hybrid 2.0.command、维护切换器和恢复入口。
...
8. 日常使用双击 Start Codex Hybrid 2.0.command。它会检查 19030 bridge、启动
   19032 hot router，再打开 ChatGPT/Codex。Codex Model Switcher.command 只用于维护。

需要 Python 3.10 或更高版本。如果电脑没有 Python，安装器会打开官方 macOS
Python 下载页，并提示安装完成后重新双击安装器。本包默认不内置 Python。

当前分发的完整本地 macOS 包包含 payload/models/local-gemma 和 payload/llama.cpp：
- 不需要云端 API key。
- 不需要手动选择 GGUF/mmproj。
- 会先复制本地模型并通过 local smoke 后再允许切换。

本包不安装 CC Switch，不包含 API key，也不包含原版 ChatGPT/Codex 应用。
