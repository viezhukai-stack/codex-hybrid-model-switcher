...
1. Install and sign in to the official ChatGPT/Codex Desktop app first.
   Official page: https://developers.openai.com/codex/app
   Current macOS builds may appear as ChatGPT.app in Applications; older builds
   may appear as Codex.app.
...
5. The installer creates a private config, validates it, runs guarded dry-run,
   and installs Start Codex Hybrid 2.0.command plus maintenance and restore
...
8. For normal daily use, double-click Start Codex Hybrid 2.0.command. It checks
   the lightweight bridge, starts the dynamic hot router on 127.0.0.1:19032,
   and opens ChatGPT/Codex. Codex Model Switcher.command is maintenance-only.
...
The distributed full local macOS package includes payload/models/local-gemma
and payload/llama.cpp. It can configure local-gemma without a cloud API key.

No macOS package includes CC Switch, API keys, or ChatGPT/Codex Desktop. Install
it only from the official page.
