...
6. 如果安装器旁边有 provider-preset.json，服务地址、模型名和 API key 环境变量名会自动预填。
7. 按提示输入你的 OpenAI-compatible 服务地址、模型名、API key 环境变量名和 API key。API key 只会写入 Windows 用户环境变量，不会写进项目配置文件。
...
12. 如果安装失败，双击 Codex Hybrid Diagnostics.cmd，然后把桌面生成的诊断 txt 发回来。
