...
5. Enter your OpenAI-compatible provider base URL, model id, API key environment variable name, and API key when prompted.
6. For local models, choose your own GGUF model file and mmproj file. This package does not include model files.
7. The installer uses bundled llama.cpp if payload\llama.cpp contains llama-server.exe; otherwise it downloads llama.cpp from the official ggml-org GitHub releases when local model files are selected.
8. The default run stops at dry-run. It does not apply a real Codex switch unless you explicitly pass -Apply and Codex Desktop is fully closed.
