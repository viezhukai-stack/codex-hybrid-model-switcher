...
12. After setup, use Start Codex Hot Router.cmd on the Desktop for normal daily launches. The shortcut uses a stable current-release pointer instead of one permanently embedded release path. It checks for a staged Microsoft Store update, waits for three identical Store observations, completes the current-user AppX registration while Codex is closed, waits again for a newly staged build, and allows at most two registration passes. If Store registration fails but the matching official package is already staged locally, it registers that package for the current user and continues. It then hash-checks the complete matching CLI bundle and aligns Browser/Chrome. Codex CLI 0.149.0 and newer reserve openai-bundled for Codex Desktop, so the updater preserves that marketplace instead of removing or re-adding it; legacy CLIs keep the explicit refresh. The launcher then checks the lightweight local bridge on 127.0.0.1:19030, starts the hot router on 127.0.0.1:19032, and opens Codex. A healthy installation takes the fast path. After Codex feature initialization, one bounded background check verifies Browser again; it never restarts Codex.
...
15. If the daily launcher's automatic update transaction stops after a Codex update, fully quit Codex and double-click Repair Codex Update and Plugins.cmd. It completes the official Store registration, copies and verifies the complete matching CLI bundle, and aligns Browser/Chrome. On CLI 0.149.0+, the reserved marketplace remains Desktop-owned; legacy versions use the official CLI refresh. Repair Codex Browser and CLI.cmd remains the narrower CLI-only fallback.
