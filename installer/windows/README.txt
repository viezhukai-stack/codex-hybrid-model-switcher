...
12. After setup, use Start Codex Hot Router.cmd on the Desktop for normal daily launches. It checks Browser/CLI update compatibility, automatically refreshes the complete matching CLI bundle after a supported Codex AppX version change while Codex is closed, then checks the lightweight local bridge on 127.0.0.1:19030, starts the hot router on 127.0.0.1:19032, and opens Codex. After Codex feature initialization, one bounded background check restores the official Browser plugin only when it is available but missing or stale; it never restarts Codex.
